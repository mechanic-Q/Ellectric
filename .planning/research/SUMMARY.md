...
- **epftoolbox LEAR model reimplementation:** ✅ 已解决 — Phase 2 重规划确定使用 sklearn.linear_model.Lasso 实现 LEAR（替代 epftoolbox 的 TensorFlow 版本）。epftoolbox 降级为基准数据源 + DM/GW 统计检验工具，不再安装在同一环境。

- **ASSUME 中国电力市场配置:** Phase 2 重规划确定优先配置中国省间现货市场规则（报价上下限、偏差考核、新能源优先调度）。通过 YAML 配置自定义中国场景，无官方模板但可参考 EPEX 配置改编。
